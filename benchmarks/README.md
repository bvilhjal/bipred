...
| `qc_factorial.py` | **Which QC steps matter**: 2x2x2 over strict thresholds, long-range LD exclusion and the LD-consistency screen, on three real trait pairs spanning the sign range; records bipred and LDSC on identical variants (→ `qc_factorial.csv`). ~75 min, same inputs as `real_ldl_cad.py` plus GLGC HDL/TG and GIANT height | — |
