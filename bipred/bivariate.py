...
# The compiled sweeps live in their own module; they are re-exported here
# because the driver, sibling modules and the tests address them through
...
def _validate_bivariate_options(*, ld_int8, h2_init,
    ...
    return _BivariateOptions(
        ...
        trace_burn_in=bool(trace_burn_in),
    )
...
class BivariateResult:
    ...
    warning; these heuristics are not a convergence certificate. With
    ``trace_burn_in=True`` the burn-in sweeps' mixture draws and raw
    quadratics are kept as ``burn_in_pi_samples`` and
    ``burn_in_genetic_samples`` for trace plots only.
    ...
    # Burn-in states, kept only with trace_burn_in=True and used by no
    # estimate: (burn_in, 4) mixture draws and (burn_in, 3) raw quadratics,
    # or, on a pooled multi-chain posterior, every chain's in chain order.
    burn_in_pi_samples: Optional[np.ndarray] = None
    burn_in_genetic_samples: Optional[np.ndarray] = None
...
def ldpred3_auto_bivariate_blocks(blocks, beta_hat1, beta_hat2, n_eff1, n_eff2, *,
                                  ...
                                  trace_burn_in=False, progress=None):
    ...
    seed : int or None
    trace_burn_in : bool, default False
        Also keep the mixture draw and the raw genetic quadratics of every
        burn-in sweep, as ``burn_in_pi_samples`` (``burn_in`` x 4) and
        ``burn_in_genetic_samples`` (``burn_in`` x 3) on the result, so a
        trace plot can show whether the start was forgotten. They enter no
        estimate: every summary still pools only the retained sweeps.
...
def _ldpred3_auto_bivariate_prepared_inner(prepared, options, start,
    ...
    # Burn-in states are kept only on request, for trace plots; they are read
    # by no estimate below.
    trace_burn_in = options.trace_burn_in
    burn_in_pi_samples = np.zeros((burn_in, 4)) if trace_burn_in else None
    burn_in_genetic_samples = np.zeros((burn_in, 3)) if trace_burn_in else None
    ...
    for it in range(burn_in + num_iter):
        ...
        if trace_burn_in and it < burn_in:
    ...
    return BivariateResult(beta1_est=beta1_mean, beta2_est=beta2_mean,
                           ...
                           burn_in_pi_samples=burn_in_pi_samples,
                           burn_in_genetic_samples=burn_in_genetic_samples)
