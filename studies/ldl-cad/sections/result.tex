...
the cap taken off the fit. LD Score regression keeps it throughout, because its
linear model is what needs it: an uncapped large-effect variant holds near-full
leverage on the slope. So the runs differ in one thing only, and are named for
it --- \texttt{fit\_chi2\_cap} is \emph{capped} or \emph{uncapped}, and every
estimate in both is bipred's. The dashed line above is the uncapped fit.
